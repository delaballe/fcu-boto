...
supported by fcu_boto. By default, this will pull in the endpoints from fcu_botocore,
...
To overwrite the existing endpoints.json file in fcu_boto:
...
from fcu_boto.endpoints import BotoEndpointResolver
from fcu_boto.endpoints import StaticEndpointBuilder
...
def load_endpoints(endpoints_file):
    ...
    try:
        ...
        print("Could not import fcu_botocore, make sure it's installed or "
              "provide an endpoints file in order to regen endpoint data.")
        sys.exit(1)

    fcu_botocore_dir = os.path.dirname(fcu_botocore.__file__)
    endpoints_file = os.path.join(fcu_botocore_dir, 'data', 'endpoints.json')
    with open(endpoints_file) as f:
        return json.load(f, object_pairs_hook=OrderedDict)
...
def main():
    ...
    parser.add_argument(
        ...
        help=('Path to endpoints.json.  If this argument is not given, '
              'then the endpoints.json file bundled with fcu_botocore will be '
              'used.'))
    ...
    exclusive.add_argument(
        ...
        help=('Overwrite the endpoints file built into fcu_boto2. This is not '
