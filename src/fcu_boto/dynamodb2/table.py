import fcu_boto
from fcu_boto.dynamodb2 import exceptions
from fcu_boto.dynamodb2.fields import (HashKey, RangeKey,
                                   AllIndex, KeysOnlyIndex, IncludeIndex,
                                   GlobalAllIndex, GlobalKeysOnlyIndex,
                                   GlobalIncludeIndex)
from fcu_boto.dynamodb2.items import Item
from fcu_boto.dynamodb2.layer1 import DynamoDBConnection
from fcu_boto.dynamodb2.results import ResultSet, BatchGetResultSet
from fcu_boto.dynamodb2.types import (NonBooleanDynamizer, Dynamizer, FILTER_OPERATORS,
                                  QUERY_OPERATORS, STRING)
from fcu_boto.exception import JSONResponseError
...
class Table(object):
    ...
    def __init__(self, table_name, schema=None, throughput=None, indexes=None,
        ...
        **IMPORTANT** - If you're creating a new ``Table`` for the first time,
        ...
        Example::
            ...
            >>> from fcu_boto import dynamodb2
    ...
    def lookup(self, *args, **kwargs):
        ...
        with fcu_boto.dynamodb. Unlike get_item, it takes hash_key and range_key first,
    ...
    def new_item(self, *args):
        ...
        This is mostly for consistency with fcu_boto.dynamodb
        """
        if not self.schema:
            self.describe()
        data = {}
...
class BatchTable(object):
    ...
    def handle_unprocessed(self, resp):
        if len(resp.get('UnprocessedItems', [])):
            ...
            fcu_boto.log.info(msg % len(unprocessed))
    ...
    def resend_unprocessed(self):
        ...
        while len(self._unprocessed):
            ...
            fcu_boto.log.info("Sending %s items" % len(to_resend))
            resp = self.table.connection.batch_write_item(batch_data)
            self.handle_unprocessed(resp)
            fcu_boto.log.info(
