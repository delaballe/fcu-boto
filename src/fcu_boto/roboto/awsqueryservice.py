...
import urlparse
import fcu_boto
import fcu_boto.connection
import fcu_boto.jsonresponse
import fcu_boto.exception
from fcu_boto.roboto import awsqueryrequest

class NoCredentialsError(fcu_boto.exception.BotoClientError):

    def __init__(self):
        s = 'Unable to find credentials'
        super(NoCredentialsError, self).__init__(s)

class AWSQueryService(fcu_boto.connection.AWSQueryConnection):
